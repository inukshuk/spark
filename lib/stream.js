...
export async function report (source, pairs, tapper) {
  ...
  await Promise.all(pairs.map(([reporter, dest]) => {
    ...
    stream.pipe(dest)
    let wait = (dest === process.stdout || dest === process.stderr) ? 'end' : 'finish'
    return new Promise((resolve, reject) => {
      ;(wait === 'end' ? stream : dest).on(wait, resolve)
...
export function combine (sources) {
  if (sources.length === 0) return new PassThrough({ objectMode: true }).end()
...
export function tag (source, process) {
  if (!source?.pipe) return source
