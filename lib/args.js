...
export function parse (args) {
  ...
  if (positionals.length)
    values.globPatterns = positionals.map(expandDirPattern)

  if (values.main) {
    values.mainGlobPatterns = values.main.map(expandDirPattern)
    delete values.main
  }

  if (values.renderer) {
    values.rendererGlobPatterns = values.renderer.map(expandDirPattern)
    delete values.renderer
  }

  if (values.concurrency != null) {
    let c = values.concurrency
    values.concurrency =
      c === 'true' ? true : c === 'false' ? false : Number(c)
  }

  if (values.verbose != null)
    values.verbose = Array.isArray(values.verbose)
      ? values.verbose.length
      : Number(values.verbose) || 0
...
function expandDirPattern (pattern) {
  try {
    if (statSync(pattern).isDirectory())
      return `${pattern}/**/*.{cjs,js,mjs}`
  } catch {}
  return pattern
}
