...
async function runTests ({
  ...
  let reporters = await resolveReporters(reporter, destination)

  return report(source, reporters, tap('test:summary'))
}
...
function resolveReporters (names, destinations) {
  return Promise.all(names.map(async (name, i) => {
    let reporter = (name in reporters)
      ? reporters[name]
      : (await import(pathToFileURL(resolve(name)))).default

    return [reporter, resolveDestination(destinations[i])]
  }))
}
