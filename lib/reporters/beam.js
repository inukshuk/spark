...
import { colors, indent, formatDuration, formatErrorMessage, formatOutput, formatStack, relativePath, sym } from './symbols.js'
...
export default class BeamReporter extends Transform {
      ...
      case 'test:summary':
        if (data.file === undefined) {
          ...
          return this.#formatSummary(data.counts, data.duration_ms) +
            this.#formatFailures() +
            formatOutput(this.#pad, this.#output)
        }
        break
    }
  }
  ...
  #formatSummary (counts, duration_ms) {
    ...
    if (this.#output.length && !config.verbose)
      out += `${pad}${dim(PASS)} ${dim('use --verbose to see captured output')}\n`
    return out
  }
  ...
  #formatFailures () {
    ...
    if (this.#failedTests.length) {
      ...
      for (let test of this.#failedTests) {
        ...
        if (loc)
          out += `${pad}${colors.dim(PASS)} ${colors.dim(`${prefix}${loc}`)}\n`
