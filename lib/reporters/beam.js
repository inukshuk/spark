...
import { colors, indent, formatDuration, formatError } from './symbols.js'
...
export default class BeamReporter extends Transform {
  ...
  #beam (status) {
    ...
    if (status === 'skip') return `${this.#pad}${colors.gray(SKIP)}`
  ...
  #formatResult (type, data) {
    ...
    let duration = data.details?.duration_ms != null
      ? colors.gray(` (${formatDuration(data.details.duration_ms)})`)
      : ''
    let suffix = ''
    let status
    ...
    if (skip !== undefined) {
      ...
      if (typeof skip === 'string' && skip.length)
        suffix = `\n${this.#beam('skip')} ${' '.repeat(data.nesting * 2)}  ${colors.gray(`# ${skip}`)}`
  ...
  #formatSummary (counts, duration_ms) {
    ...
    if (skipped)
      out += `${pad}${colors.gray(SKIP)} ${dim(`${skipped} skip`)}\n`
      ...
      for (let test of this.#failedTests) {
        ...
        if (test.details?.error) {
          ...
          for (let line of err.split('\n'))
            out += `${pad}${colors.gray(PASS)}   ${line}\n`
        }
      }
      this.#failedTests = []
    }
    out += `${pad}${colors.gray(END)}\n`
    return out
  }
