...
const { pass: PASS, fail: FAIL, skip: SKIP, end: END } = sym.beamline

export default class BeamlineReporter extends Transform {
  #stack = []
  #failedTests = []
  #output = []
  #summaryDone = false
  ...
  #handleEvent ({ type, data }) {
    switch (type) {
      ...
      case 'test:diagnostic':
        ...
        return `${this.#beamline('pass')} ${' '.repeat(data.nesting * 2)}${colors.blue(data.message)}\n`
      case 'test:summary':
  ...
  #formatResult (type, data) {
    ...
    while (this.#stack.length) {
      ...
      prefix += `${this.#beamline('suite')} ${' '.repeat(parent.data.nesting * 2)}${parent.data.name}\n`
    }
    ...
    if (skip !== undefined) {
      ...
      if (typeof skip === 'string' && skip.length)
        suffix = `\n${this.#beamline('skip')} ${' '.repeat(data.nesting * 2)}  ${colors.dim(`# ${skip}`)}`
    } else if (todo !== undefined) {
      status = 'skip'
      if (typeof todo === 'string' && todo.length)
        suffix = `\n${this.#beamline('skip')} ${' '.repeat(data.nesting * 2)}  ${colors.yellow(`# ${todo}`)}`
    ...
    let name = status === 'fail'
      ...
      : status === 'skip'
        ...
        : data.name
    return `${prefix}${this.#beamline(status)} ${nest}${name}${duration}${suffix}\n`
  }

  #formatSummary (counts, duration) {
    let pad = this.#pad
    let dim = colors.dim
    ...
    out += `${pad}${dim(PASS)} ${dim(`in ${formatDuration(duration)}`)}\n`
