...
function emitBeforeExit (stream) {
  ...
  // If no files were enqueued try to exit right away.
  setImmediate(() => {
    if (pending === 0) process.emit('beforeExit')
  })
}
...
export function createTestRunner ({
  ...
  switch (isolation) {
    ...
    case 'process': {
      ...
      Object.assign(opts, childOpts({ preload, ...opts }))
      break
    }
  }
...
function childOpts ({
  ...
  execArgv = [
    ...
    ...preload.flatMap(module => ['--require', module]),
    ...execArgv
  ]
