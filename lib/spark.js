...
import { filterStdErr, combine, tag } from './stream.js'
...
export function run ({
  ...
  if (main || !renderer)
    streams.push(tag(run.runMain({ ...opts, ...main }), 'main'))
  if (renderer)
    streams.push(
      ...run.runRenderer({ ...opts, ...renderer }).map(s => tag(s, 'renderer')))
...
export function runMain ({
  ...
  debug('spark:main', {
    files, globPatterns, isolation, preload, execArgv
  })
...
export function runRenderer ({
  ...
  debug('spark:renderer', {
    files, globPatterns, isolation, preload
  })
